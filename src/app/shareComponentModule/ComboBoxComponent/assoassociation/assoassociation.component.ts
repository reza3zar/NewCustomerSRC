...
export const CUSTOM_INPUT_CONTROL_VALUE_ACCESSOR: any = {
  ...
  useExisting: forwardRef(() => AssociationComponent ),
  multi: true
};
...
@Component({
  selector: 'ngx-associationComponent',
...
export class AssociationComponent  implements OnInit , OnDestroy{
  ...
  ngOnInit() {
    if (AssociationComponent.associationsCollection.length <= 0)
    this.getassociationsCollection();
  else this.dataassociations = AssociationComponent.associationsCollection;
  }
...
public selectionChange(value: any): void {
  ...
  getassociationsCollection() {
    this.localSubscriber= this.comboService.getAssociation().subscribe(data => {
      AssociationComponent.associationsCollection = data.slice();
      this.dataassociations = AssociationComponent.associationsCollection;
    });
  }

  handleFilterassociations(value) {
    this.dataassociations = AssociationComponent.associationsCollection.filter(
