...
@Component({
  selector: 'ngx-attorney-requests-management',
  templateUrl: './attorney-requests-management.component.html',
  styleUrls: ['./attorney-requests-management.component.scss']
})
export class AttorneyRequestsManagementComponent implements OnInit,OnDestroy {
  ...
  ngOnInit() {
    ...
    refreshRequests(){
      ...
      this.brokerRequestsSubscriber = this.attorneyService.getAttorneyRequests().subscribe(result=>
        {
          console.log(result)
...
checkIsSelectedRow(itemId: number,isOperational:boolean=true) {
  ...
  if (this.selectedRequestItemResult.id == itemId){
    if(this.selectedRequestItemResult.requestStatus.id==2 || this.selectedRequestItemResult.requestStatus.id==4  || this.selectedRequestItemResult.requestStatus.id==8)
     {
      return false;
     }
  }
...
public selected(e) {
  let selectedCredit = new AttorneyRequest();
  this.disableShowBtn = false;
  this.isClicked=false;

  this.selectedRequestItemResult = e.selectedRows[0]
    ? (e.selectedRows[0].dataItem as AttorneyRequest)
    : new AttorneyRequest(); 
...
ngOnDestroy(): void {
  ...
  if (this.requestsSendRequestSubscriber !== undefined) {
    this.sidebarSubscriber.unsubscribe();
  }
...
requestsSendRequestSubscriber: Subscription;
 
sendRequestToServer(){
  this.sendDataToServer=true;
  // this.checkIsSelectedRow(this.selectedRequestItemResult.id);
  this.requestsSendRequestSubscriber=  this.attorneyService.sendAttorneyRequest(   this.selectedRequestItemResult.id.toString()).subscribe(result =>{
...
openConfirmDeleteModal(){
  ...
  getSelectedAttorneyRequest(typeOpereation){
    this.getAttorneyRequestByRequestIdSubscriber= this.attorneyService.getAttorneyRequestByRequestId(this.selectedRequestItemResult.id).subscribe(result=>
      {
  ...
  goToDocumentsManagement(id,data){
    ...
    this.router.navigate(['broker/documentsManagement/',id,  CustomerTypeEnum.UnKnown,data.attorneyName,false,DocumentTypeEnum.Attorney,1]);
  }
